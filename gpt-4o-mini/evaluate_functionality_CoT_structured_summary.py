...
cot_variant_dir = os.path.join(batch_dir, "CoT_variant_structured_summary")
...
def parse_test_inputs(filepath: str) -> List[str]:
    ...
    if sample_match:
        inputs.append(sample_match.group(1).strip())
...
def evaluate(filename: str, path_cot: str) -> dict:
...
def evaluate_all():
    ...
    for fname in os.listdir(cot_variant_dir):
        if not fname.endswith(".py"):
            continue
        path_cot = os.path.join(cot_variant_dir, fname)

        res = evaluate(fname, path_cot)
        if res:
            results.append(res)

    out_path = os.path.join(batch_dir, "functionality_results_structured_summary.csv")
    with open(out_path, "w", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=["problem_id", "cot_passed", "total"])
        writer.writeheader()
