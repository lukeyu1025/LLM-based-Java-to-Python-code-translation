...
def generate(prompt: str, max_new_tokens=512) -> str:
...
- Do NOT add variable names or assignments.
...
"""
    ...
    print(f" Generating test inputs for {problem_id} using Gemma...")

    output_text = generate(prompt)
    output_text = output_text.replace("```", "")  # Clean out undesired Markdown formatting
...
if __name__ == "__main__":
    import sys
    if len(sys.argv) != 2:
        print("Usage: python generate_testcase_gemma.py <problem_id>")
    else:
